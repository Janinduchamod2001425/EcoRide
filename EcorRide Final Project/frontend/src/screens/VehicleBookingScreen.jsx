...
const VehicleBookingScreen = () => {
    ...
    const vehicle = params.get('license') || '';
    const category = params.get('category') || '';
    const model = params.get('model') || '';
    const price = parseFloat(params.get('price')) || 0;

    const [add, { isLoading }] = useAddbookingMutation();
    ...
    const submitHandler = async (e) => {
        e.preventDefault();
        try {
            const res = await add({ vehicle, category, model, reserve_date, duration, total_price }).unwrap();
            ...
            toast.info('If you want to update the Reservation Details \n Put a request to admin.', {
                position: "top-right",
                autoClose: 7000,
                hideProgressBar: false,
                style: {
                ...
                <div className='licenseForm'>
                    <Form className='licenseCard' onSubmit={submitHandler}>
                        ...
                        {isLoading ? (
                            ...
                            <Button type='submit' variant='primary' id='BookingBTN'>Submit Booking</Button>
                        )}
