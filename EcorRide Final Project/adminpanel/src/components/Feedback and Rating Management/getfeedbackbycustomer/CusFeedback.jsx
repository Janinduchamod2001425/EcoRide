...
const CusFeedback = () => {
    ...
    useEffect(() => {
        const fetchFeedbacks = async (customerId) => {
          try {
            const response = await axios.get(`http://localhost:8000/api/feedbacks/customer/${customerId}`);
      ...
      }, [customerId]); // Trigger the effect whenever customerId changes
    ...
    const handleSearchChange = (e) => {
        ...
        const filtered = feedbacks.filter((feedback) => {
            const { age, gender, district, preferred_package, usage, suggestion, find, type, comment, rating } = feedback;
            return (
                age.toLowerCase().includes(query) ||
                gender.toLowerCase().includes(query) ||
                district.toLowerCase().includes(query) ||
                preferred_package.toLowerCase().includes(query) ||
                usage.toString().includes(query) ||
                suggestion.toString().includes(query) ||
                find.toString().includes(query) ||
                type.toString().includes(query) ||
                comment.toString().includes(query) ||
    ...
    return (
        <div className="feedbackTable">
            <h2 className="headingTitle">Manage your Feedbacks</h2>
            ...
            <div className="buttons-container">
                ...
                <Link to={"/addfeedback"} className='addButton'>Add New Feedback</Link>
                    ...
                    {filteredFeedbacks.map((feedback) => (
                        <tr key={feedback._id}>
                            ...
                            <td className="actionButtons">
                                <Link to={`/editbooking/` + feedback._id}>Edit</Link>
                            </td>
                            <td className="actionButtons">
